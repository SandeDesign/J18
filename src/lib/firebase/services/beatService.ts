...
class BeatService {
  ...
  async getBeatBySlug(slug: string): Promise<Beat | null> {
    try {
      const q = query(
  ...
  async searchBeats(searchTerm: string): Promise<Beat[]> {
    try {
      const allBeatsQuery = query(
        collection(db, this.collectionName),
        where('status', '==', 'published')
      );
      const querySnapshot = await getDocs(allBeatsQuery);

      const beats: Beat[] = [];
      querySnapshot.forEach((doc) => {
        const beat = { id: doc.id, ...doc.data() } as Beat;
        const searchLower = searchTerm.toLowerCase();

        const matchesTitle = beat.title.toLowerCase().includes(searchLower);
        const matchesGenre = beat.genre.toLowerCase().includes(searchLower);
        const matchesArtist = beat.artist.toLowerCase().includes(searchLower);
        const matchesTags = beat.tags.some(tag => tag.toLowerCase().includes(searchLower));

        if (matchesTitle || matchesGenre || matchesArtist || matchesTags) {
          beats.push(beat);
        }
      });
  ...
  async updateBeat(id: string, data: Partial<Beat>): Promise<void> {
    const user = authService.getCurrentUser();
    if (!user || user.role !== 'admin') {
      throw new Error('Unauthorized: Only admins can update beats');
    }
  ...
  async deleteBeat(id: string): Promise<void> {
    const user = authService.getCurrentUser();
    if (!user || user.role !== 'admin') {
      throw new Error('Unauthorized: Only admins can delete beats');
    }
  ...
  async toggleLike(id: string): Promise<void> {
    try {
      const beat = await this.getBeatById(id);
  ...
  subscribeToBeat(id: string, callback: (beat: Beat | null) => void): Unsubscribe {
    return onSnapshot(
      doc(db, this.collectionName, id),
      (doc) => {
        if (doc.exists()) {
          callback({ id: doc.id, ...doc.data() } as Beat);
        } else {
          callback(null);
        }
      },
      (error) => {
        console.error('Subscribe to beat error:', error);
        callback(null);
      }
    );
  }
